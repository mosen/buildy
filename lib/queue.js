 ...
 * - promise: the emitter object that hands control over to the task, and takes it back when complete.
 * (TODO rename promise to StatusEmitter)
 */
...
Queue.prototype._fork = function(forkspec) {
    ...
    this._logger.log('info', '[fork] creating fork(s)...');
    ...
    Object.keys(forkspec).forEach(function eachFork(queue_name) {
        ...
        self._logger.log('debug', '[fork] created new fork: ' + queue_name);
...
Queue.prototype._exec = function(type, spec, promise) {
    if (this.registry.taskExists(type)) {
        this.registry.task(type).apply(this, [spec, promise]);
    } else {
...
Queue.prototype._onTaskComplete = function(type, result) {
    ...
    console.log(this._queuePosition);
    ...
    this._logger.log('info', '[' + this._name + '][' + this._queue[this._queuePosition].type + '] Task complete: ' + result);
