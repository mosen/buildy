...
//    glob   = require('glob'), // unsupported by node v0.5, using minimatch instead
...
Buildy.prototype = {
    ...
    files : function(filespec, promise) {
        ...
        filespec.forEach(function(f) {
            ...
            if (/(^!|^#|[?*])/.test(f)) {
                promise.emit('failure', 'files', 'glob support has been temporarily removed.');
            } else {
                fnAddFiles([f]);
            }
        });
    ...
    concat : function(spec, promise) { 
        switch (this._type) {
            ...
            case Buildy.TYPES.FILES:
                ...
                this._type = Buildy.TYPES.STRING;
                promise.emit('complete', 'concat', this._state.join(', '));
                break;
        ...
        switch (this._type) {
            ...
            case Buildy.TYPES.FILES:
                this._state.forEach(function(f) {
                    utils.lint({sourceFile: f, options: lintOptions}, function(result) {
            ...
            case Buildy.TYPES.STRING:
                ...
                utils.lint({source: this._state, options: lintOptions}, function(result) {
                    var reporter = require('jslint/lib/reporter.js');
            ...
            case Buildy.TYPES.STRINGS:
                ...
                this._state.forEach(function(f) {
                    utils.lint({source: f, options: lintOptions}, function(result) {
                        var reporter = require('jslint/lib/reporter.js');
                        reporter.report('buildy', result);
            ...
            fnWriteFile = function fnWriteFile() {
                ...
                fs.writeFile(spec.name, self._state, 'utf8', function(err) {
                    if (err) { 
                        ...
                        promise.emit('complete', 'write', self._type.join(', '));
                    }
                });               
            };
