 ...
 * @param spec {Object} Task specifications, describes how the task will act, what kind of inputs it takes etc.
 * @public
 */
Registry.prototype.add = function(name, spec) {
    //util.log('Registering task: ' + name);

    if (this._isTaskDefinitionValid(spec)) {
        this._tasks[name] = spec;
    } else {
        util.log('Can\'t register task "' + name + '", specification is invalid.');
    }
};
 ...
 * @param spec {Object} Task specification to validate
 * @return {Boolean} True if the specification is valid.
 * @private
 */
Registry.prototype._isTaskDefinitionValid = function(spec) {
    if (spec.hasOwnProperty('callback') && typeof spec.callback === "function") {
        return true;
    } else {
        return false;
    }
