 ...
 *
 * @method jsminify
 * @param spec {Object} Minify task options/specification
 * @param spec.encoding {String} [encoding='utf8'] Encoding used for files and strings.
 * @param spec.parse_strict_semicolons {Boolean} [parse_strict_semicolons=false] Throw an error when semicolon is not found
 * @param spec.mangle {Boolean} [mangle=true] Mangle variable and function names
 * @param spec.mangle_options {Object} [mangle_options={}] Options for ast_mangle, see UglifyJS @ github.
 * @param spec.lift_variables {Boolean} [lift_variables=false] Hoist variable declarations to the top of their scope.
 * @param spec.squeeze {Boolean} [squeeze=true] Perform compression using various optimizations
 * @param spec.squeeze_options {Object} [squeeze_options={}] Options for ast_squeeze, see UglifyJS @ github.
 * @param spec.options {Object} [options={}] Options for the code generator, see UglifyJS @ github.
...
function jsminifyTask(spec, promise) {
    ...
    for (prop in spec) {
        if (spec.hasOwnProperty(prop)) {
            default_options[prop] = spec[prop];
        }
    }
    ...
    switch (this._state.get().type) {
        case State.TYPES.STRING:
            uglify(this._state.get().value, '', default_options, function(err, data) {
                if (err) {
                    promise.emit('failed', 'jsminify', 'failed to minify string: ' + err);
                } else {
                    self._state.set(State.TYPES.STRING, data);
                    promise.emit('success', 'jsminify', 'minified string');
                }
            });
            break;
        ...
        case State.TYPES.STRINGS:
            ...
            function uglifyStringBatchDone(stringcontents) {
                ...
                promise.emit('success', 'jsminify', 'minified ' + stringCountTotal + ' string(s).');
            }

            function uglifyStringDone(err, data, filename) {
                // TODO: fail on error?
                uglified.unshift(data);
                if (!--fileCount) {
                    uglifyStringBatchDone(uglified);
                }
            }
        ...
        case State.TYPES.FILES:
            ...
            function uglifyBatchDone(filecontents) {
                ...
                promise.emit('success', 'minify', 'Finished minifying');

            }

            function uglifyDone(err, data, filename) {
                // TODO: fail on error?
                uglified[filename] = data;
                if (!--fileCount) {
                    uglifyBatchDone(uglified);
                }
            }
