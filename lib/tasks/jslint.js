 ...
 * @param spec.encoding [spec.encoding='utf8'] Encoding used for strings and files.
 * @param spec.* Lint options
...
function jslintTask(spec, promise) {
    ...
    switch (this._state.get().type) {
        ...
        case State.TYPES.FILES:
            ...
            promise.emit('complete', 'jslint', 'Started linting ' + this._state.get().value.length + ' file(s).');
            break;
