 ...
 * @param spec {Object} task options
 * @param spec.paths {Array} paths to be searched for LESS import directives.
 * @param spec.encoding {String} [encoding='utf8'] Encoding to use for strings and files.
...
function cssminifyTask(spec, promise) {
    ...
    spec = spec || {};
    spec.encoding = spec.encoding || 'utf8';
    ...
    switch (this._state.get().type) {
        case State.TYPES.FILES:
            ...
            function fileBatchMinified(contents) {
                ...
                promise.emit('success', 'cssminify', 'Minified ' + filesToProcessTotal + ' file(s).');
            }
            ...
            this._state.get().value.forEach(function(filename) {
                fs.readFile(filename, spec.encoding, function(err, data) {
                    if (err) {
                        // TODO: fail task?
                        fileMinified('Couldnt minify file: ' + filename + ', reason: ' + err);
                    } else {
                        less(data, filename, spec, fileMinified);
                    }
                });
            });
            break;
        ...
        case State.TYPES.STRINGS:
            ...
            function stringBatchMinified(contents) {
                ...
                promise.emit('success', 'cssminify', 'Minified ' + stringsToProcessTotal + ' string(s).');
            }
            ...
            this._state.get().value.forEach(function(str) {
                less(str, '', spec, stringMinified);
            });
            break;

        case State.TYPES.STRING:
            less(this._state.get().value, '', spec, function(err, data) {
                self._state.set(State.TYPES.STRING, data);
                promise.emit('success', 'cssminify', 'Minified CSS string.');
            });
            break;

        default:
            promise.emit('failed', 'cssminify', 'unrecognised input type: ' + this._state.get().type);
            break;
    }
}
