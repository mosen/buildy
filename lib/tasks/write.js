 ...
 *
 *  - FILES
 *  Write the list of files to the specified file (line feed after each filename).
 *
 *  - STRINGS
 *  Write the strings (concatenated) to the specified file.
 *
 *  - STRING
 ...
 * @public
 */
function writeTask(options, promise) {
    var self = this,
    ...
    function writeFile(filename, data) {
        fs.writeFile(filename, data, encoding, function(err) {
            if (err) {
                promise.emit('failed', 'write', 'error writing destination file: ' + err);
            } else {
                self._state.set(State.TYPES.FILES, [ filename ]);
                promise.emit('success', 'write', 'successfully wrote ' + filename);
            }
        });
    }
    ...
    switch (this._state.get().type) {
        case State.TYPES.FILES:
            mkdirIfNotExist(pathname, function(err) {
                if (err) {
                    promise.emit('failed', 'write', 'error creating destination directory: ' + err);
                } else {
