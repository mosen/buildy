 ...
 */
function concatTask(params) {
    var destination = params && params.dest || null,
        encoding    = this.state.meta('encoding') || 'utf8';

    var stateItemCount = this.state.length();

    switch (this.state.type()) {

        case 'STRING':
            if (this.state.length() === 1) {
                this.emit('complete', 'concat', 'did not attempt to concatenate single string.');
            } else {
                this.state.set(this.state.value().join(''));
                this.emit('complete', 'concat', 'concatenated ' + stateItemCount + ' item(s).');
            }
            break;

        case 'FILE':
            if (this.state.length() === 1) {
                this.emit('complete', 'concat', 'did not attempt to concatenate a single file.');
            } else {
                var concatenated = concatFilesSync(destination, this.state.value(), this.state.meta('encoding'));
                this.state.set(concatenated);
                this.emit('complete', 'concat', 'concatenated ' + stateItemCount + 'file(s).');
            }
            break;
    }
}
