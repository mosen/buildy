...
var Copy = function copy(src, dst, callback) {
    ...
    function _validateDest() {
        fs.exists(path.dirname(dst), function(dst_exists) {
            if (!dst_exists) {
    ...
    sources.forEach(function eachCopyItem(item) {
        ...
        if (/(^!|^#|[?*])/.test(item)) {
            ...
            newGlob.on('end', function(pattern) {
                self.emit('parseGlobDone', pattern);
            });
        } else {
            // TODO: isNotExcluded is called twice, once here and once at .add
 ...
 * @return {EventEmitter} copy events emitter
 */
...
/**
 * Copy an array of source files/patterns/directories to a single destination.
 *
 * The callback function takes two parameters:
 * - err: Whether there was an error encountered while copying.
 * - results: Object with properties 'complete', 'failed' which are arrays containing the result of each copy operation.
 *
 * @method cprf
