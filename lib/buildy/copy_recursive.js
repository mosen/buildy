...
CopyRecursive.prototype.initialize = function () {
    ...
    function _getRelativePath(base, fullPath) {
        if (base === fullPath) { return path.basename(fullPath); }

        if (fullPath.substr(0, base.length) == base) {
            return fullPath.substr(base.length);
        }
    }
