...
function Copy(src, dst, callback, options) {
        ...
        path.exists(path.dirname(dst), function(dst_exists) {
            if (!dst_exists) {
                if (options && options.mkdir === false) {
                    self.emit('error', new Error('Destination ' + dst + ' does not exist, and mkdir is false. aborting copy'));
                    return;
                } else {
                    ...
                    mkdirp(pathAbs, permissions, function(err) {
                        if (err) {
                            self.emit('error', 'Could not create destination: ' + err);
                            return;
                        }
    ...
    function _validateFiles() {
        fs.stat(src, function(err, stat) {
            ...
            if (stat.isFile()) {
                ...
                self.emit('error', new Error('copying of sockets, pipes, and devices is not supported: ' + src));
            }
        });
    }
    ...
    function _openInputFile() {
        fs.open(src, 'r', function(err, infd) {
            ...
            if (err) {
                self.emit('error', new Error('Error reading source file: ' + err));
                return;
            }
    ...
    function _openOutputFile(infd) {
        fs.open(dst, 'w', function(err, outfd) {
            ...
            if(err) {
                self.emit('error', new Error('Error opening destination for write: ' + err));
                return;
            }
