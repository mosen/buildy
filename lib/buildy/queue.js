...
Queue.prototype._fork = function(forkspec) {
    ...
    Object.keys(forkspec).forEach(function eachFork(queue_name) {
        var clonedState = new State(),
            q = new Queue(queue_name, { // TODO: each time we add a new property to the queue this has to be extended
                registry: self.registry, // Cloning the queue in a fork should be taken care of by some other process
                state: clonedState,
                logger: self._logger,
                skip: self._skip,
