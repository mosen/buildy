 ...
 */
Queue.prototype.fork = function(forkParams) {
    ...
    Object.keys(forkParams).forEach(function eachFork(name) {
        ...
        forks.push(q);
        forkParams[name].call(q); // Execute the next fork in the context of the child queue.
    });
};
...
Queue.prototype._exec = function(type, params, status) {
    if (this.registry.taskExists(type)) {
        ...
        try {
            this.registry.task(type)[this.state.type()].apply(this, [params, status, this._logger]);
        } catch (e) {
            status.emit('failed', 'Got an error trying to execute task: ' + type);
        }
    } else {
