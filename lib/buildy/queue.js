...
var util        = require('util')
    ...
    , State     = require('./state')
    , isArray = util.isArray || function(ar) { // Workaround for util.isArray not available
 ...
 * @param {winston.Logger} options.logger Instance of winston logger to use.
 * @param {Array} options.skip Array of task names to skip.
 * @param {Object} options.defaults Object properties as task names with a value object that corresponds to that task's
 * parameters.
 * @param {Registry} options.registry Instance of registry to use.
 * @param {State} options.state Instance of state to use.
 */
var Queue = module.exports = function Queue(name, options) {
     ...
     *
     * Should contain all parent queues that this queue
     * has been forked from. If this queue has been constructed directly
     ...
     *
     * @property registry
     * @default an instance of registry with default paths (buildy built-in tasks only).
     * @private
     */
    this.registry = options.registry || new Registry({ autoload: true });

    /**
     * Object containing the state (file content + metadata) to be modified by the tasks.
     *
     * @property state
     * @default empty state.
     * @public
 ...
 * @return {Queue} A reference to this Queue object, for chaining methods.
 * @public
 */
 ...
 * all configuration from the parent queues except for the name, which is
 * derived from the literal object property names.
 *
 * @method fork
